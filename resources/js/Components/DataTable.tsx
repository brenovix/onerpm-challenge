...
import { AlertCircle, Check, CircleX, Loader2, RefreshCw, X } from 'lucide-react';
...
const DataTable: React.FC<DataTableProps> = ({
  ...
  const renderCell = (col: Column, row: any) => {
    ...
    if (typeof value === 'boolean') {
      return (
        <span className={`inline-flex items-center px-1.5 py-0.5 w-8 h-8 rounded-full text-xs font-medium ${
          value 
            ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300' 
            : 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300'
        }`}>
          {value ? <Check className='h-8' /> : <X className='h-8' /> }
        </span>
      );
    }
